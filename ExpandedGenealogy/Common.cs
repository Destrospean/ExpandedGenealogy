...
namespace Destrospean.ExpandedGenealogy
{
    ...
    public static class Common
    {
        ...
        public static AncestorInfo GetStepAncestorInfo(GenealogyPlaceholder stepDescendant, GenealogyPlaceholder stepAncestor)
        {
            List<AncestorInfo> ancestorInfoList = new List<AncestorInfo>();
            ...
            while (tempAncestorInfoAndParentList.Count > 0)
            {
                ...
                else if (tempParent.IsAncestor(stepAncestor) || tempParent.Genealogy.IsStepAncestor(stepAncestor.Genealogy))
                {
                    parents = new List<GenealogyPlaceholder>();
                    if (Tuning.kAccumulateStepRelatives || stepDescendant.IsAncestor(tempParent))
                    {
                        parents.AddRange(tempParent.Genealogy.GetStepParents());
                    }
        ...
        public static List<GenealogyPlaceholder> GetStepAncestors(this Genealogy sim)
        {
            List<GenealogyPlaceholder> ancestors = new List<GenealogyPlaceholder>(), stepAncestors = new List<GenealogyPlaceholder>(), tempAncestors = new List<GenealogyPlaceholder>();
            tempAncestors.AddRange(sim.GetStepParents());
            tempAncestors.AddRange(sim.GetGenealogyPlaceholder().Parents);
            while (tempAncestors.Count > 0)
            {
                GenealogyPlaceholder ancestor = tempAncestors[0];
                tempAncestors.RemoveAt(0);
                ancestors.Add(ancestor);
                if (Tuning.kAccumulateStepRelatives || sim.GetGenealogyPlaceholder().IsAncestor(ancestor)) 
                {
                    tempAncestors.AddRange(ancestor.Genealogy.GetStepParents());
                }
                tempAncestors.AddRange(ancestor.Parents);
            }
            foreach (GenealogyPlaceholder ancestor in ancestors)
            {
        ...
        public static List<GenealogyPlaceholder> GetStepParents(this Genealogy sim)
        {
            List<GenealogyPlaceholder> stepParents = new List<GenealogyPlaceholder>();
            foreach (Genealogy parent in sim.Parents)
            {
                if (parent.Spouse != null && !Genealogy.IsParent(parent.Spouse, sim))
                {
                    stepParents.Add(parent.Spouse.GetGenealogyPlaceholder());
                }
            }
            return stepParents;
        }
        ...
        public static List<Genealogy> GetStepSiblings(this Genealogy sim)
        {
            ...
            foreach (GenealogyPlaceholder stepParent in sim.GetStepParents())
            {
                foreach (Genealogy child in stepParent.Genealogy.Children)
                {
        ...
        {
            return self.GetStepAncestors().Contains(other.GetGenealogyPlaceholder());
        }
